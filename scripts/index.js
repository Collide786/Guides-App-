...
const setupUI = (user) => {
  ...
  if (user) {
    ...
    db.collection('users').doc(user.uid).get().then(doc => {
        ...
        <img id="profileImage" style="width: 125px; border-radius: 1%;" alt="not working" src="${image}"> </img>
      ...
      accountDetails.innerHTML = html;
        ...
        html = `
          <img id="profileImage" style="width: 125px; border-radius: 1%;" src="${noImgSrc}"> </img>
          <br/>
          <br/>
          <div style="font-size: 1.2rem;">Logged in as ${user.email}</div>
          <div style="font-size: 1.2rem;">Bio: ${doc.data().bio}</div>
